...
class ParallelFileExtractor:
    """
    Handles parallel file extraction using ThreadPoolExecutor
    """

    def __init__(self, max_workers: Optional[int] = None, batch_size: int = 4, verbose: bool = False):
        self.max_workers = max_workers or min(8, cpu_count())
    ...
    def _extract_single_file(self, args: Tuple) -> Tuple[bool, str, int]:
        """Extract a single file in a worker thread"""
        file_info, output_path, ipf_path = args

        try:
            # Open IPF file for extraction (each worker needs its own handle)
            with open(ipf_path, 'rb') as file:
                with zipfile.ZipFile(file) as ipf_zip:
                    with ipf_zip.open(file_info.zip_info, pwd=self.password) as member_file:
            ...
            try:
                fallback_name = f"fallback_{file_info.index:04d}.bin"
                fallback_path = os.path.join(os.path.dirname(output_path), fallback_name)
                ...
                with self._lock:
                    self._extracted_count += 1
                return True, fallback_name, file_info.zip_info.file_size

            except Exception as fallback_error:
                if self.verbose:
                    print(f"✗ Fallback also failed for {file_info.safe_name}: {fallback_error}")
                return False, file_info.safe_name, 0

    def extract_files(self, ipf_path: str, file_infos: List[FileInfo], output_dir: str) -> bool:
        """Extract all files in parallel batches"""
        print(f"Extracting {len(file_infos)} files using {self.max_workers} workers "
        ...
        for file_info in file_infos:
            ...
            extraction_tasks.append((unique_file_info, output_path, ipf_path))
...
def process_ipf_file_parallel(ipf_path: str, output_dir: str = "extracted_parallel",
                             max_workers: Optional[int] = None, verbose: bool = False) -> bool:
    """
    Extract IPF file with parallel filename decryption and file extraction
    """
    print(f"Processing '{ipf_path}' in parallel mode...")
    ...
    try:
        ...
        file_extractor = ParallelFileExtractor(max_workers, verbose=verbose)
        success = file_extractor.extract_files(ipf_path, processed_files, output_dir)

        print(f"\n✓ Parallel extraction completed! Files saved to: {output_dir}")
        return True
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python ipf_extractor_parallel.py <file.ipf> [output_dir] [max_workers] [--verbose|-v]")
        print("\nThis tool extracts IPF files with parallel filename decryption and extraction.")
        ...
        print("\nExamples:")
        print("  python ipf_extractor_parallel.py file.ipf")
        print("  python ipf_extractor_parallel.py file.ipf output 4")
        print("  python ipf_extractor_parallel.py file.ipf output 4 --verbose")
        sys.exit(1)

    ipf_file = sys.argv[1]
    output_dir = "extracted_parallel"
    max_workers = None
    verbose = False
    ...
    success = process_ipf_file_parallel(ipf_file, output_dir, max_workers, verbose)
