...
def get_ipf_password():
    """
    Return the static IPF password
    """
    # Same bytes used in the common ipfpasswword.txt found in other tools and hardcoded in ez.exe
    return bytes([
...
class ZipCipher:
    """
    Implements the ZIP stream cipher for decrypting filenames and data
    Based on the PKWARE encryption algorithm
    """
    def __init__(self):
        # CRC32 table
        self.crc32_tab = [
    ...
    def init_keys(self, password):
        """Initialize encryption keys with password"""
        self.keys = [0x12345678, 0x23456789, 0x34567890]  # Initial values

        for byte_val in password:
            self.update_keys(byte_val)
    ...
    def crc32(self, c, b):
        ...
        tmp2 = c >> 8
        tmp &= 0xff
        return self.crc32_tab[tmp] ^ tmp2

    def update_keys(self, byte_val):
        """Update encryption keys"""
        self.keys[0] = self.crc32(self.keys[0], byte_val)
        self.keys[1] += self.keys[0] & 0xff
        self.keys[1] = (self.keys[1] * 134775813) & 0xffffffff
        self.keys[1] += 1
        keyshift = self.keys[1] >> 24
        self.keys[2] = self.crc32(self.keys[2], keyshift)

    def decrypt_byte(self):
        """Generate next decryption byte"""
    ...
    def decrypt_data(self, data):
        ...
        for byte_val in data:
            decrypted_byte = byte_val ^ self.decrypt_byte()
...
def decode_filename_from_local_header(file, file_info):
    ...
    cipher = ZipCipher()
    cipher.init_keys(get_ipf_password())
    ...
    # Fallback: try to detect Japanese encoding
    try:
...
def make_safe_filename(filename):
    ...
    safe = parts[-1]  # Just use the filename part
